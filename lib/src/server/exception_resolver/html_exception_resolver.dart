...
class HtmlExceptionResolver implements ExceptionResolver, ApplicationContextAware, InitializingPod {
  ...
  ///
  /// This list contains all `@ControllerAdvice`
  ...
  /// {@template default_rest_controller_adviser}
  /// Default [ControllerAdviser] for `@RestControllerAdvice`.
  ///
  /// This adviser combines:
  /// - The [RestControllerAdvice] annotation.
  /// - The [RestControllerExceptionHandler] class type.
  /// - The static [RestControllerExceptionHandler.CLASS] reference for Pod lookup.
  ///
  /// It is used internally to provide framework-level exception handling
  /// for REST controllers, ensuring consistent behavior without requiring
  /// user registration.
  /// {@endtemplate}
  ...
  Future<bool> resolve(ServerHttpRequest request, ServerHttpResponse response, HandlerMethod? handler, Object ex) async {
    final contentType = request.getHeaders().getContentType() ?? WebUtils.producing(handler?.getMethod()).firstOrNull ?? MediaType.TEXT_HTML;
    final exceptionClass = ex.getClass();
    final cached = getCacheableHandler(handler, exceptionClass);
    final isAnnotated = handler is AnnotatedHandlerMethod && handler.definition.annotation is! RestController;
    final isWeb = isAnnotated || handler is WebViewHandlerMethod;

    if (handler != null && isWeb && cached != null) {
    ...
    if (errorPage != null) {
      ...
      await _returnValueHandler.handleReturnValue(errorPage, null, request, response, handler);
      return true;
    }

    final acceptHeader = request.getHeaders().getAccept();
    final expectsHtml = acceptHeader.isNotEmpty || acceptHeader.any((acc) => acc.includes(contentType) || acc.isCompatibleWith(contentType));

    if (expectsHtml || isWeb) {
      response.getHeaders().setContentType(contentType);
      ...
      final errorPage = _errorPages.findPossibleErrorPage(status) ?? ErrorPage.ERROR_INTERNAL_SERVER_PAGE;
...
///
/// ### Usage
/// Used internally by [HtmlExceptionResolver] to cache the
