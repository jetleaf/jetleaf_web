...
class RestExceptionResolver implements ExceptionResolver, ApplicationContextAware, InitializingPod {
  ...
  @override
  Future<bool> resolve(ServerHttpRequest request, ServerHttpResponse response, HandlerMethod? handler, Object ex, st) async {
    ...
    final isRest = resolvedHandler is AnnotatedHandlerMethod && resolvedHandler.definition.annotation is RestController;

    if (WebUtils.renderAsJson(response) || isRest) {
      WebUtils.resolveMediaTypeAsJson(response);
      
      if (definition != null && resolvedHandler != null) {
        ...
        final res = ResponseBody.of(exception.getStatus(), exception);
        await _returnValueHandler.handleReturnValue(res, null, request, response, resolvedHandler);
