...
/// {@template yaml_return_value_handler}
/// [ReturnValueHandler] implementation that handles **YAML-serializable**
/// controller return values.
///
/// This handler activates for controller methods intended to return YAML
/// responses and automatically serializes the return value as YAML using
/// an appropriate [HttpMessageConverter].
///
/// ### Overview
/// When a handler returns an object with YAML media type negotiation,
...
///
/// {@endtemplate}
final class YamlReturnValueHandler implements ReturnValueHandler {
  /// Central registry of [HttpMessageConverter] instances.
  final HttpMessageConverters _converters;

  /// {@macro yaml_return_value_handler}
  YamlReturnValueHandler(this._converters);
  ...
  bool canHandle(Method? method, Object? returnValue, ServerHttpRequest request) {
    ...
    // Check if method explicitly produces YAML
    if (method != null) {
  ...
  @override
  List<Object?> equalizedProperties() => [YamlReturnValueHandler];

  @override
  List<MediaType> getSupportedMediaTypes() => [
    MediaType.APPLICATION_YAML,
    MediaType.APPLICATION_XYAML,
    MediaType('text', 'yaml'),
  ...
  Future<void> handleReturnValue(Object? returnValue, Method? method, ServerHttpRequest request, ServerHttpResponse response, HandlerMethod? hm) async {
    ...
    final yamlConverters = _converters.getMessageConverters()
        .where((c) => c.getSupportedMediaTypes().any((mt) => mt.getType() == 'application' && mt.getSubtype() == 'yaml'))
        .toList();

    if (yamlConverters.isEmpty) {
      throw HttpMediaTypeNotSupportedException('No suitable YAML HttpMessageConverter found for ${valueClass.getName()}');
    }
